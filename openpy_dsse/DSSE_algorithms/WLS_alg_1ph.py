...
#from observability_analysis import observability_LU_Decomposition
from .Symb_Eqn.sym_func_1ph import *
...
class WLS_1ph_state_estimator:
    ...
    def WLS_nonlinear_linear_PMU(self):
        ...
        'SCADA measurements'
        matrix_calc = Zm_hcalc_Jacobian(vi, pi, qi, pf, qf, ii, nvi, npi, nqi, npf, nqf, nii)
        ...
        V1_Mag = np.array([Matrix_Mang_Ang_initial[:, 3]]).T
        Ang1_rdn = np.array([Matrix_Mang_Ang_initial[:, 4]]).T
        E = np.concatenate((Ang1_rdn[1:, :1], V1_Mag), 0)
        ...
        while tol > self.tolerance:
            ...
            h_calculate = matrix_calc.h_calculate(fbus_m, tbus_m, V1_Mag, Ang1_rdn, nbus, self.YBus_Matrix)
            'residual vector between the vector z(m) existing measurements and h(x) calculated measurements'
            residual = Z_MEAS - h_calculate
            # Jacobian..
            # SCADA measurements
            H = matrix_calc.Jacobian(fbus_m, tbus_m, V1_Mag, Ang1_rdn, nbus, self.YBus_Matrix)
            # Gain Matrix, Gm..
        ...
        if self.DF_MEAS_PMU.empty == True:
            ...
            V1_Mag = np.array(V1_Mag).astype(None)
            Ang1_deg = np.array(Ang1_deg).astype(None)
            DSSE_results = np.concatenate((bus_name, np.round(V1_Mag, 4), np.round(Ang1_deg, 4)), 1)
            ...
            V1_Mag = np.array(V1_Mag).astype(None)
            Ang1_rdn = np.array(Ang1_rdn).astype(None)

            CvE = np.array([np.diag(np.linalg.inv(H.T @ np.linalg.inv(Rii_total) @ H))]).T
            CvE = np.diagflat(np.concatenate((CvE[nbus - 1:], CvE[0:nbus - 1]), 0))
            R11 = np.diagflat(np.cos(Ang1_rdn))
            R12 = np.concatenate((np.zeros([1, nbus - 1]), np.diagflat(-V1_Mag[1:] * np.sin(Ang1_rdn[1:]))), 0)
            R21 = np.diagflat(np.sin(Ang1_rdn))
            R22 = np.concatenate((np.zeros([1, nbus - 1]), np.diagflat(-V1_Mag[1:] * np.cos(Ang1_rdn[1:]))), 0)
            R = np.concatenate((np.concatenate((R11, R12), 1), np.concatenate((R21, R22), 1)), 0)
            W1 = np.array([np.diag(R @ CvE @ R.T)]).T
            W1[nbus] = 1e-20

            m_pmu = np.array(self.DF_MEAS_PMU)
            m_type = np.array([m_pmu[:, 0]]).T
            magn = np.array([m_pmu[:, 1]]).T.astype(None)
            ang = np.array([m_pmu[:, 2]]).T.astype(None)
            fbus_pmu = np.array([m_pmu[:, 5]]).T
            tbus_pmu = np.array([m_pmu[:, 6]]).T
            Rim = np.array([m_pmu[:, 7]]).T
            Ria = np.array([m_pmu[:, 7]]).T

            vi_pmu = np.argwhere(m_type == 7)
            ii_pmu = np.argwhere(m_type == 8)

            nvi_pmu = len(vi_pmu)
            nii_pmu = len(ii_pmu)

            num_MEAS['Vi|θ'] = nvi_pmu
            num_MEAS['Iij|δ'] = nii_pmu

            Ang1_rdn = (math.pi / 180) * Ang1_rdn

            Esr = V1_Mag * np.cos(Ang1_rdn)
            Esi = V1_Mag * np.sin(Ang1_rdn)
            Vpr = magn[0:nvi_pmu] * np.cos(ang[0:nvi_pmu])
            Vpi = magn[0:nvi_pmu] * np.sin(ang[0:nvi_pmu])
            Ipr = magn[nvi_pmu:] * np.cos(ang[nvi_pmu:])
            Ipi = magn[nvi_pmu:] * np.sin(ang[nvi_pmu:])
            M = np.concatenate((Esr, Esi, Vpr, Vpi, Ipr, Ipi), 0)  # Measurement Vector.

            # Forming new Jacobian Matrix
            J11 = np.eye(nbus, nbus)
            J12 = np.zeros([nbus, nbus])
            J21 = J12
            J22 = J11

            J31 = np.zeros([nvi_pmu, nbus])
            for i in range(0, nvi_pmu):
                m = int(fbus_pmu[int(vi_pmu[i, 0])]) - 1
                for k in range(0, nbus):
                    if k == m:
                        J31[i, k] = 1

            J32 = np.zeros([nvi_pmu, nbus])
            J41 = J32
            J42 = J31

            J51 = np.zeros([nii_pmu, nbus])
            for i in range(0, nii_pmu):
                m = int(fbus_pmu[int(ii_pmu[i, 0])]) - 1
                n = int(tbus_pmu[int(ii_pmu[i, 0])]) - 1
                for k in range(0, nbus):
                    if k == m:
                        G1, B1 = get_G_B_Pos_seq(m, n, self.YBus_Matrix)
                        J51[i, k] = -G1
                    elif k == n:
                        G1, B1 = get_G_B_Pos_seq(m, n, self.YBus_Matrix)
                        J51[i, k] = G1

            J52 = np.zeros([nii_pmu, nbus])
            for i in range(0, nii_pmu):
                m = int(fbus_pmu[int(ii_pmu[i, 0])]) - 1
                n = int(tbus_pmu[int(ii_pmu[i, 0])]) - 1
                for k in range(0, nbus):
            ...
            J1 = np.concatenate((J11, J12), 1)
            J2 = np.concatenate((J21, J22), 1)
            J3 = np.concatenate((J31, J32), 1)
            J4 = np.concatenate((J41, J42), 1)
            J5 = np.concatenate((J51, J52), 1)
            J6 = np.concatenate((J61, J62), 1)
            Hc = np.concatenate((J1, J2, J3, J4, J5, J6), 0)

            # Rotation Matrix..
            R11 = np.diagflat(np.cos(ang[0:nvi_pmu]))
            R12 = np.diagflat(-magn[0:nvi_pmu] * np.sin(ang[0:nvi_pmu]))
            R21 = np.diagflat(np.sin(ang[0:nvi_pmu]))
            R22 = np.diagflat(magn[0:nvi_pmu] * np.cos(ang[0:nvi_pmu]))
            R31 = np.diagflat(np.cos(ang[nvi_pmu:]))
            R32 = np.diagflat(-magn[nvi_pmu:] * np.sin(ang[nvi_pmu:]))
            R41 = np.diagflat(np.sin(ang[nvi_pmu:]))
            R42 = np.diagflat(magn[nvi_pmu:] * np.cos(ang[nvi_pmu:]))

            R1 = np.concatenate((R11, R12), 1)
            R2 = np.concatenate((R21, R22), 1)
            R3 = np.concatenate((R31, R32), 1)
            R4 = np.concatenate((R41, R42), 1)
            Rv = np.concatenate((R1, R2), 0)
            Ri = np.concatenate((R3, R4), 0)

            Wv = np.concatenate((Rim[0:nvi_pmu], Ria[0:nvi_pmu]), 0)
            Wi = np.concatenate((Rim[nvi_pmu:], Ria[nvi_pmu:]), 0)
            W2 = np.concatenate((Rv ** 2 @ Wv, Ri ** 2 @ Wi), 0)
            W = np.diagflat(np.concatenate((W1, W2), 0)).astype(None)

            R = np.array([np.diag(np.linalg.inv(W))]).T
            R = np.diagflat(R)

            E = np.linalg.inv(Hc.T @ R @ Hc) @ (Hc.T @ R) @ M

            V1_Mag = np.sqrt(E[nbus:] ** 2 + E[0:nbus] ** 2)
            # list containing x and y coordinates
            x = E[0:nbus]
            y = E[nbus:]
            Ang1_rdn = np.zeros([len(x), 1])
            for i in range(len(x)):
                Ang1_rdn[i] = math.atan2(y[i], x[i])
            Ang1_rdn = Ang1_rdn * 180 / math.pi
            DSSE_results = np.concatenate((bus_name, np.round((V1_Mag), 4), np.round((Ang1_rdn), 4)), 1)
            DF_DSSE_results = pd.DataFrame(DSSE_results, columns=['Bus Nro.', 'V1(pu)_EST', 'Ang1(deg)_EST'])

            return DF_DSSE_results, n_Iter + 1, tol, num_MEAS
    ...
    def WLS_nonlinear_PMU(self):
        ...
        'Creation and ordering of the vector of SCADA measurements'
        'SCADA measurements'
        matrix_calc_SCADA = Zm_hcalc_Jacobian(vi, pi, qi, pf, qf, ii, nvi, npi, nqi, npf, nqf, nii)
        matrix_calc_PMU = Zm_hcalc_Jacobian_PMU(vi_pmu, ii_pmu, nvi_pmu, nii_pmu)
        ...
        DF_DSSE_results = pd.DataFrame(DSSE_results, columns=['Bus Nro.', 'V1(pu)_EST', 'Ang1(deg)_EST'])
        DSSE_results = np.array(DF_DSSE_results)

        return DSSE_results, DF_DSSE_results, n_Iter, tol, num_MEAS

class Zm_hcalc_Jacobian:

    def __init__(
            self, vi: int, pi: int, qi: int, pf: int, qf: int, ii: int,
            nvi: int, npi: int, nqi: int, npf: int, nqf: int, nii: int):
...
class Zm_hcalc_Jacobian_PMU:
    ...
    def Jacobian_PMU(self, fbus_pmu, tbus_pmu, Vsp, theta, nbus, YBus_Matrix):
        ...
        Jacobian_PMU = np.concatenate((H7, H8, H9, H0), 0)
